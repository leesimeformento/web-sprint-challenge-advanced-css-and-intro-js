...
function getArtistByIndex(array, index) {
  /* code here */
  return `The artist index ${array[index].id} is ${array[index].name}`
}
...
dataArr.forEach(item => {
    const yr = item.years.split(" - ")
    let bornYr = parseInt(yr[0])
    let deathYr = parseInt(yr[1])

    if (bornYr >= 1900 && deathYr <= 2000) {
        arr.push(item.name)
    }
